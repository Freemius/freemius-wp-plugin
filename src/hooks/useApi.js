...
export function useApi( endpoint, options = {} ) {
	...
	const refetch = useCallback(
		async ( forceRefresh = false, isRetryAttempt = false ) => {
			...
			if ( ! isRetryAttempt ) resetErrorState();
...
export function useApiPagination( endpoint, options = {} ) {
	...
	const hook = useApi( endpoint, {
		...
		immediate: true,
		onSuccess: ( response ) => {
			if ( currentPage === 1 ) setAllData( response.items || response );
			else
				setAllData( ( prev ) => [
					...prev,
			...
			if ( hookOptions.onSuccess ) hookOptions.onSuccess( response );
		},
	} );
