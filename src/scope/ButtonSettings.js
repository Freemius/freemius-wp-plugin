...
const ButtonSettings = ( props ) => {
	...
	const closeCheckout = () => {
		...
		iframeDoc
			.getElementById( 'fs-checkout-page-' + handler.guid )
			?.remove();
	...
	const openCheckout = () => {
		...
		data_copy.cancel = function () {
			setPreview( false );
			setLoading( false );
			if ( merged.cancel ) new Function( merged.cancel ).apply( this );
		};

		if ( merged.purchaseCompleted )
			data_copy.purchaseCompleted = function ( data ) {
				new Function( 'data', merged.purchaseCompleted ).apply( this, [
					data,
				] );
			};
		...
		data_copy.track = function ( event, data ) {
			...
			if ( merged.track )
				new Function( 'event', 'data', merged.track ).apply( this, [
					event,
					data,
				] );
		};
