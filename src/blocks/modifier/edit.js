...
export default function Edit( props ) {
	...
	useEffect( () => {
		...
		if ( current !== newCurrent ) setAttributes( { current: newCurrent } );
	}, [ contextWithModifications[ type ], isLoading, isLoadingModifiers ] );

	const changeScope = ( property, value ) => {
		const newModifications = { ...scopeAttributes?.freemius_modifications };
		...
		else newModifications[ property ] = value;
...
const ModifierToggles = ( props ) => {
	...
	const { disabled = [], labels = {} } = attributes;
	...
	const setLabel = ( option, value ) => {
		...
		else newLabels[ option.id ] = value;

		if ( Object.keys( newLabels ).length === 0 )
			setAttributes( { labels: undefined } );
		else setAttributes( { labels: newLabels } );
	};
